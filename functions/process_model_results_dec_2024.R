...
process_model_results <- function(){
  ...
  for(pg in 1:length(pgs)){
    ...
    fn <- file.path(dir_out1, paste0(gsub(" ", "_", species),
  ...
  # save
  write.csv(agg_indices, file.path(dir_out1, paste0(gsub(" ", "_", species), 
                                "_relative_abundance_indices.csv")), 
            na="", row.names=F)
  write.csv(agg_trends, file.path(dir_out1, paste0(gsub(" ", "_", species), 
